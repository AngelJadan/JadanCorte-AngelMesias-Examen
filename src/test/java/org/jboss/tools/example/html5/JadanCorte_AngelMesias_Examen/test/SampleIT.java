package org.jboss.tools.example.html5.JadanCorte_AngelMesias_Examen.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/JadanCorte-AngelMesias-Examen.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "JadanCorte-AngelMesias-Examen.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("org.jboss.tools.example.html5.JadanCorte_AngelMesias_Examen.test");
